...
public class ExpressionMapper {
    ...
    private StringToken mapToStringToken(final StringNode token) {
       // return new StringToken(stringNode.getField(), stringNode.getValue(),stringNode.getOperator());
        return StringToken.builder().field(token.getField()).value(token.getValue()).operator(Operator.getOperatorFromSymbol(token.getOperator()).orElse(null)).build();
    }
    ...
    private NumericRangeToken mapToNumericRangeToken(final NumericRangeNode token) {
        return NumericRangeToken.builder().field(token.getField())
                .fromValue(token.getFromValue())
